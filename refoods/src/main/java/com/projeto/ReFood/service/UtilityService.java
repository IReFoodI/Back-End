...
public class UtilityService {
  ...
  public void associateReview(Consumer<Review> reviewSetter, Long reviewId) {
    if (reviewId != null) {
      Review review = reviewRepository.findById(reviewId)
          .orElseThrow(() -> new NotFoundException());
      reviewSetter.accept(review);
    } else {
      throw new IllegalArgumentException("Review ID não pode ser nulo ao criar um review.");
    }
  }

  public void associateTransaction(Consumer<Transaction> transactionSetter, Long transactionId) {
    if (transactionId != null) {
      Transaction transaction = transactionRepository.findById(transactionId)
          .orElseThrow(() -> new NotFoundException());
      transactionSetter.accept(transaction);
    } else {
      throw new IllegalArgumentException("Transaction ID não pode ser nulo ao criar um transaction.");
    }
  }
  ...
  public void addAddressToRestaurant(Restaurant restaurant,
      @NotNull(message = "Obrigatório") @Valid() AddressDTO addressDTO) {
